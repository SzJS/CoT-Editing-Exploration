...
- **vLLM `--served-model-name`**: When serving local MXFP4 models with vLLM, always pass `--served-model-name "gpt-oss-20b"` (or similar short name). Without it, vLLM uses the full filesystem path as the model ID, and `eval_cot.py` gets 404 "model does not exist" errors when sending that path in API requests. Then pass `--model="gpt-oss-20b"` to `eval_cot.py` (and `--tokenizer=<path>` separately for the tokenizer).
